...
interface MaintenanceTaskDao {
    ...
    @Query("""
        SELECT t.* FROM maintenance_tasks t
        JOIN vehicles v ON t.vehicleId = v.id
        WHERE t.isDeleted = 0 AND v.isArchived = 1 AND v.isDeleted = 0
        ORDER BY t.createdAt DESC
    """)
    fun observeArchived(): Flow<List<MaintenanceTaskEntity>>
    ...
    @Query("SELECT * FROM maintenance_tasks WHERE vehicleId = :vehicleId AND isDeleted = 0")
    suspend fun getForVehicle(vehicleId: String): List<MaintenanceTaskEntity>
    ...
    @Query("SELECT * FROM maintenance_tasks WHERE associatedRecordId = :recordId LIMIT 1")
    suspend fun getByAssociatedRecordId(recordId: String): MaintenanceTaskEntity?
    ...
    @Query("UPDATE maintenance_tasks SET completed = :completed, associatedRecordId = :recordId, isSynced = 0 WHERE id = :id")
    suspend fun setCompleted(id: String, completed: Boolean, recordId: String?)
